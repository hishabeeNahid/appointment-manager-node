...
describe('Authentication Routes', () => {
  ...
  describe('POST /api/v1/auth/register/patient', () => {
    ...
    it('should return error for invalid email format', async () => {
      ...
      expect(response.body.success).toBe(false);
      expect(response.body.errorMessages[0].message).toBe('Invalid email format');
    });
    ...
    it('should return error for short password', async () => {
      ...
      expect(response.body.success).toBe(false);
      expect(response.body.errorMessages[0].message).toBe('Password must be at least 6 characters');
    });
  ...
  describe('POST /api/v1/auth/register/doctor', () => {
    ...
    it('should return error when specialization is missing', async () => {
      ...
      expect(response.body.success).toBe(false);
      expect(response.body.message).toBe('Specialization is required for doctors');
    });
  ...
  describe('Password Security', () => {
    it('should hash passwords during registration', async () => {
      ...
      const isValidPassword = await bcrypt.compare(testPatient.password, user!.password);
